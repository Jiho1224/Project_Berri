...
def movie_rev():
    ...
    global good_topic_time
    global good_topic_reviews
    global good_topic_good
    global good_topic_bad
    global bad_topic_time
    global bad_topic_reviews
    global bad_topic_good
    global bad_topic_bad
    ...
    good_reviewIndex, bad_reviewIndex = [], []
    for num in range(10):
        indexList = [i for i in range(len(clean_reviews)) if topic[num] in [word for word, tag in sentences_tag[i]]]
        # print("indexList ", indexList)
        scoreList = []
        for index in indexList:
            new_sentence = clean_reviews[index]
            new_sentence = re.sub(r'[^ㄱ-ㅎㅏ-ㅣ가-힣 ]', '', new_sentence)
            new_sentence = okt.morphs(new_sentence, stem=True)
            new_sentence = [word for word in new_sentence if not word in desc.movie_stop_word()]
            tokenizer.fit_on_texts([new_sentence])
            encoded = tokenizer.texts_to_sequences([new_sentence])
            pad_new = pad_sequences(encoded, maxlen=35)
            score = float(loaded_model.predict(pad_new))
            scoreList.append(score)
        print("scoreList ", num, scoreList)
        good_reviewIndex.append(indexList[scoreList.index(max(scoreList))])
        bad_reviewIndex.append(indexList[scoreList.index(min(scoreList))])


    good_topic_time, good_topic_reviews, good_topic_good, good_topic_bad = [], [], [], []
    bad_topic_time, bad_topic_reviews, bad_topic_good, bad_topic_bad = [], [], [], []
    for i in range(10):
        good_topic_time.append(time[good_reviewIndex[i]])
        good_topic_reviews.append(reviews[good_reviewIndex[i]])
        good_topic_good.append(good[good_reviewIndex[i]])
        good_topic_bad.append(bad[good_reviewIndex[i]])
        bad_topic_time.append(time[bad_reviewIndex[i]])
        bad_topic_reviews.append(reviews[bad_reviewIndex[i]])
        bad_topic_good.append(good[bad_reviewIndex[i]])
        bad_topic_bad.append(bad[bad_reviewIndex[i]])
    # print("good_topic_reviews ", good_topic_reviews)
    # print("bad_topic_reviews ", bad_topic_reviews)
    print(bad_topic_good, bad_topic_bad)

    db.number.update_one({'name': 'only'}, {'$set': {'article': num_article + 1, 'reply': num_reply + len(reviews)}})
    return jsonify({'url': url,'평점': rating, '날짜': time, '댓글': reviews, '공감': good, '비공감': bad, '논란지수': controversy,
                    ...
                    'topic_good': topic_good, 'topic_bad': topic_bad,'good_topic_time':good_topic_time, 'good_topic_reviews':good_topic_reviews, 'good_topic_good':good_topic_good, 'good_topic_bad':good_topic_bad,
                    'bad_topic_time':bad_topic_time, 'bad_topic_reviews':bad_topic_reviews, 'bad_topic_good':bad_topic_good, 'bad_topic_bad':bad_topic_bad})
